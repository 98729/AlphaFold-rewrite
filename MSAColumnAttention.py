...
class MSAColumnAttention:
    ...
    def __call__(self, msa_act, msa_mask, q_data, m_data, is_training=False):
        ...
        q = np.einsum('bqa,ahc->bqhc', q_data, q_weights) * key_dim ** (-0.5)
        k = np.einsum('bka,ahc->bkhc', m_data, k_weights)
        v = np.einsum('bka,ahc->bkhc', m_data, v_weights)
