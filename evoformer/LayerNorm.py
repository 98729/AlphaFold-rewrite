...
def to_axes_or_slice(axis):
    if isinstance(axis, slice):
        ...
        return (axis,)
    elif np.iterable([axis]) and all(isinstance(ax, int) for ax in axis):
        return tuple(axis)
    else:
        raise ValueError(f"`axis` should be an int, slice or iterable of ints. Got: {axis}")


def to_abs_axes(axis, ndim: int):
    if isinstance(axis, slice):
...
class LayerNorm:
    ...
    def __init__(self, axis: int or tuple[int], create_scale: bool, create_offset: bool,
                 eps: float = 1e-5, scale_init=1, offset_init=0, use_fast_variance: bool = False,
    ...
    def __call__(self, inputs: np.ndarray, scale=None, offset=None,) -> np.ndarray:
        ...
        if self.create_scale:
            ...
            for i in range(len(param_shape)):
                num *= param_shape[i]
            scale = np.array([self.scale_init] * num, dtype=inputs.dtype).reshape(shape=param_shape)
        elif scale is None:
        ...
        if self.create_offset:
            ...
            for i in range(len(param_shape)):
                num *= param_shape[i]
            offset = np.array([self.scale_init] * num, dtype=inputs.dtype).reshape(shape=param_shape)
        elif offset is None:
        ...
        inv = scale * np.sqrt(variance + self.eps)
